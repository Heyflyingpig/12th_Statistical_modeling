...
- 格网级评分样本数：`6373`
- `ROC-AUC = 0.7613`
- `Top 10% hit rate = 60.29%`
- `Lift@10% = 2.3750`
- `AUC >= 0.70` 和 `Lift@10% >= 2.0` 两个原设强筛查门槛均已达到

因此，下一年格网级概率图可表述为“非高风险格网升级为高风险的优先筛查图”，强调其用于年度巡查排序和重点复核。长期部分采用 Markov 状态转移矩阵做 **10 年低/中/高风险占比趋势外推**，输出 `Q3/output/q3_risk_state_projection_10yr.csv`、`Q3/output/q3_risk_state_projection_10yr.json` 和 `Q3/output/Q3_Risk_State_Projection_10yr.png`。这一部分适合作为趋势情景分析，不应写成精确到格网的 10 年预测。
...
- Q3 已完成年度 Moran/LISA、状态转移、空间 Markov、非高风险转高风险概率预测和 10 年状态占比趋势外推；其中格网级预测模块已达到原设筛查门槛，但仍应表述为“优先筛查与趋势情景分析”，不写成逐格网确定性预报。
